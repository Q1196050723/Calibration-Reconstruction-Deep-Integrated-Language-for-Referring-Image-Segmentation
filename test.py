        ...
        for data in metric_logger.log_every(data_loader, 100, header):
            ...
            for j in range(sentences.size(-1)):
                if bert_model is not None:
                    ...
                    last_hidden_states,class_token = bert_model(sentences[:, :, j], attention_mask=attentions[:, :, j])
                    embedding = last_hidden_states.permute(0, 2, 1)
                    output = model(image, embedding, class_token,l_mask=attentions[:, :, j].unsqueeze(-1))[0]
                else:
                    output = model(image, sentences[:, :, j], l_mask=attentions[:, :, j])[0]

                # output = output.cpu()
                
                # output_mask = output.data.numpy()
                I, U = IoU(output, target)
                if U == 0:
                    this_iou = 0.0
                else:
                    this_iou = I*1.0/U
